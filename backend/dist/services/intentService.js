...
exports.getIntentDistribution = exports.calculateIntentScore = void 0;
const database_1 = require("../config/database");
/**
 * Calculate intent score for a session
 */
const calculateIntentScore = async (sessionId) => {
    try {
        ...
        const intentScoreValue = (normalizedTime * 0.3) +
        ...
        await database_1.prisma.sessionIntentScore.upsert({
            ...
            update: {
                final: score100,
                peak: {
                    set: Math.max(score100, (await database_1.prisma.sessionIntentScore.findUnique({ where: { sessionId: session.id } }))?.peak || 0)
                }
            },
            create: {
        ...
        if (session.userId) {
            await database_1.prisma.userIntentScore.upsert({
 ...
 */
const getIntentDistribution = async (websiteId) => {
...
exports.getIntentDistribution = getIntentDistribution;
