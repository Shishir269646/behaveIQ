...
export default function ContentPage() {
  const { website: selectedWebsite, selectWebsite } = useAppStore((state) => ({
    ...
    const fetchOptions = async () => {
      ...
      try {
        const response = await api.get(
          `/content/options?websiteId=${selectedWebsite._id}`
        )
        ...
        setOptionsError(
          err.response?.data?.message ||
            err.message ||
            'Failed to fetch content options.'
        )
        toast({
          title: 'Error',
          description:
  ...
  const handleGenerateContent = async () => {
    ...
    if (!sessionId) {
      toast({
        title: 'Error',
        description:
          'Session ID is not available. Please try again or refresh the page.',
        variant: 'destructive',
      })
      return
    }
    ...
    setGeneratedContent('')
    try {
      const selectedPersonaObject = availablePersonas.find(
  ...
  if (!selectedWebsite) {
    return (
      <Alert variant='destructive'>
        ...
        <AlertDescription>
          No website selected. Please select a website to generate content.
        </AlertDescription>
      </Alert>
    )
  }
          ...
          <CardContent className='space-y-4'>
            <div className='space-y-2'>
              ...
              <Select
                ...
                disabled={isLoadingOptions || !availablePersonas.length}
              >
            ...
            <div className='space-y-2'>
              ...
              <Select
                ...
                disabled={isLoadingOptions || !availableContentTypes.length}
              >
