    ...
    <ErrorBoundary>
      <>
          ...
          {isLoading || !overview ? (
            ...
            <>
              <Card>
                ...
                <CardContent>
                  <div className='text-2xl font-bold'>
                    {formatNumber(overview.totalVisitors.value)}
                  </div>
                  <p className='text-xs text-muted-foreground'>
                    {formatChange(overview.totalVisitors.change)} from last
                    month
                  </p>
                </CardContent>
              </Card>
              <Card>
                ...
                <CardContent>
                  <div className='text-2xl font-bold'>
                    {formatNumber(overview.totalSessions.value)}
                  </div>
                  <p className='text-xs text-muted-foreground'>
                    {formatChange(overview.totalSessions.change)} from last
                    month
                  </p>
                </CardContent>
              </Card>
              <Card>
                ...
                <CardContent>
                  <div className='text-2xl font-bold'>
                    {formatNumber(overview.totalConversions.value)}
                  </div>
                  <p className='text-xs text-muted-foreground'>
                    {formatChange(overview.totalConversions.change)} from last
                    month
                  </p>
                </CardContent>
              </Card>
              <Card>
                ...
                <CardContent>
                  <div className='text-2xl font-bold'>
                    {overview.avgIntentScore &&
                    typeof overview.avgIntentScore.value === 'number'
                      ? overview.avgIntentScore.value.toFixed(2)
                      : '0.00'}
                  </div>
                  <p className='text-xs text-muted-foreground'>
                    {overview.avgIntentScore &&
                    typeof overview.avgIntentScore.change === 'number'
                      ? formatChange(overview.avgIntentScore.change)
                      : '+0%'}{' '}
                    since last hour
                  </p>
                </CardContent>
          ...
          {isLoading || !selectedWebsite ? ( // Use selectedWebsite to determine loading of these cards
            ...
            <>
              ...
              <Card>
                ...
                <CardContent>
                  <div className='text-2xl font-bold'>
                    {formatNumber(selectedWebsite.stats?.totalPersonas || 0)}
                  </div>
                  <p className='text-xs text-muted-foreground'>
              ...
              <Card>
                ...
                <CardContent>
                  <div className='text-2xl font-bold'>
                    {selectedWebsite.settings?.autoPersonalization
                      ? 'Enabled'
                      : 'Disabled'}
                  </div>
                  <p className='text-xs text-muted-foreground'>
        ...
        <div className='grid gap-4 md:gap-8 lg:grid-cols-2 xl:grid-cols-3'>
          ...
          !(selectedWebsite.stats as any).intentDistribution ? (
            <Skeleton className='h-96' />
          ) : (
            <IntentScoreDistributionChart
              data={(selectedWebsite.stats as any).intentDistribution}
            />
          )}
        </div>
        <div className='grid gap-4 md:gap-8 lg:grid-cols-1'>
                    ...
                    <TableBody>
                      {recentSessions.map((session) => (
                        ...
                        >
                          ...
                          <TableCell
                            data-label='Intent Score'
                            className='text-right'
                          >
                            {session.intentScore &&
                            typeof session.intentScore.final === 'number'
                              ? session.intentScore.final.toFixed(2)
                              : 'N/A'}
                          </TableCell>
                        </TableRow>
                      ))}
        ...
        {isLoading ||
        !selectedWebsite?.stats ||
        !(selectedWebsite.stats as any).insights ? ( // insights are part of website stats
