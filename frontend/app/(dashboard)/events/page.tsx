  ...
  if (!website) {
    return (
      <Alert variant='default'>
        ...
        <AlertDescription>
          Please select a website from the navigation to view events.
        </AlertDescription>
      </Alert>
    )
  }
        ...
        <CardHeader>
          ...
          <CardDescription>
            {events && events.length > 0
              ? `Displaying the ${events.length} most recent events.`
              : 'Awaiting incoming events...'}
          </CardDescription>
        </CardHeader>
        <CardContent className='overflow-x-auto'>{renderContent()}</CardContent>
      </Card>
    </div>
  )
}
