  ...
  return (
    <div className='flex flex-col gap-4'>
      ...
      <p className='text-muted-foreground'>
        Shows not just where users click, but where they hesitate and get
        confused.
      </p>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription className='flex items-center justify-between'>
            <span>
              Visualizing user clicks on the homepage over the last 7 days.
            </span>
            <div className='flex flex-col md:flex-row items-start md:items-center justify-between gap-2 md:gap-4'>
              <div className='flex items-center gap-2'>
                <Label htmlFor='heatmap-type'>Map Type</Label>
                <Select value={mapType} onValueChange={setMapType}>
                  <SelectTrigger id='heatmap-type' className='w-full md:w-auto'>
              ...
              <div className='flex items-center gap-2'>
                ...
                >
                  <SelectTrigger id='heatmap-page' className='w-full md:w-auto'>
                    <SelectValue
                      placeholder={
                        loadingPages ? 'Loading pages...' : 'Select page'
                      }
                    />
                  </SelectTrigger>
                  <SelectContent>
                    {errorPages && (
                      <SelectItem value='__error_loading_pages__' disabled>
                        Error loading pages
                      </SelectItem>
                    )}
        ...
        <CardContent>
          ...
          {heatmapData && !loading && !error && (
            <div className='w-full h-[400px] rounded-lg'>
              {mapType === 'click' && <Heatmap data={heatmapData.clicks} />}
              {mapType === 'scroll' && (
                <div className='p-4'>
                  <h3 className='font-bold text-lg'>Scroll Depth</h3>
                  <p>
                    Average Scroll Depth:{' '}
                    {heatmapData.scrollDepth.avgScrollDepth}%
                  </p>
                  <p>
                    Max Scroll Depth: {heatmapData.scrollDepth.maxScrollDepth}%
                  </p>
                </div>
              )}
              {mapType === 'confusion' && (
                  ...
                  <ul>
                    {heatmapData.confusionZones.map((zone, index) => (
                      <li key={index} className='border-b py-2'>
                        <p>
                          <strong>Element:</strong> {zone.element}
                        </p>
