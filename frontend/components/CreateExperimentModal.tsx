...
const formSchema = z.object({
  ...
  variations: z
    ...
    .min(2, 'At least two variations are required'),
  minSampleSize: z.coerce
    .number()
    .min(1, 'Minimum sample size is required')
    .optional(),
})
...
export function CreateExperimentModal({
  ...
  const [selectedExperimentWebsiteId, setSelectedExperimentWebsiteId] =
    useState<string | undefined>(website?._id)
  const [isCreateWebsiteModalOpen, setIsCreateWebsiteModalOpen] =
    useState(false)
        ...
        if (value.variations) {
          ...
          const total =
            typedVariations.reduce(
              (sum, v) =>
                sum +
                (typeof v.trafficPercentage === 'number'
                  ? v.trafficPercentage
                  : 0),
              0
            ) || 0
      ...
      if (name === 'variations') {
        if (value.variations && value.variations.length > 0) {
          ...
          if (controlCount === 0) {
            ...
            const firstControlIndex = typedVariations.findIndex(
              (v) => v.isControl
            )
            typedVariations.forEach((v, index) => {
  ...
  const onSubmit = async (values: FormInput) => {
    if (!selectedExperimentWebsiteId) {
      form.setError('name', {
        message: 'Please select a website or create a new one.',
      })
      return
    }
    ...
    if (totalTraffic !== 100) {
      form.setError('variations', {
        message: 'Total traffic percentage must be 100%',
      })
      return
    }
        ...
        >
              ...
              {websites.length > 0 ? (
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    ...
                    <Button
                      size='sm'
                      onClick={() => setIsCreateWebsiteModalOpen(true)}
                      className='ml-2'
                    >
                      Create Website
                    </Button>
          ...
          <div className='grid grid-cols-1 sm:grid-cols-4 items-center gap-4'>
            ...
            {form.formState.errors.minSampleSize && (
              <p className='col-span-4 col-start-2 text-sm text-red-500'>
                {form.formState.errors.minSampleSize.message}
              </p>
            )}
          </div>
          ...
          <div className='col-span-4'>
            ...
            <div className='grid gap-4'>
              {fields.map((field, index) => (
                <div
                  key={field.id}
                  className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-10 items-center gap-2'
                >
                  <Label
                  ...
                  />
                  <Label
                    htmlFor={`is-control-${index}`}
                    className='col-span-1 md:col-span-2 text-right'
                  >
                    Control
                  </Label>
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      form.setValue(`variations.${index}.isControl`, checked)
                    }
                  ...
                  {form.formState.errors.variations?.[index]
                    ?.trafficPercentage && (
                    <p className='col-span-10 col-start-3 text-sm text-red-500'>
                      {
                        form.formState.errors.variations[index]
                          ?.trafficPercentage?.message
                      }
                    </p>
                  )}
                </div>
