...
export const createDiscountSlice: StateCreator<
  DiscountSlice,
  [],
  [],
  DiscountSlice
> = (set, get) => ({
  ...
  fetchDiscounts: async (websiteId: string) => {
    const discounts = await handleRequest(set, () =>
      api.get(`/websites/${websiteId}/discounts`)
    )
    set({ discounts })
  },
  ...
  updateDiscount: async (
    ...
    await handleRequest(
      set,
      () => api.put(`/discounts/${discountId}`, discountData),
  ...
  deleteDiscount: async (discountId: string) => {
    const websiteId = get().discounts.find(
      (d) => d._id === discountId
    )?.websiteId // Get websiteId before deleting
    await handleRequest(
      set,
      () => api.delete(`/discounts/${discountId}`),
