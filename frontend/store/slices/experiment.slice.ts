...
> = (set, get) => ({
  ...
  fetchExperiments: async (websiteId: string) => {
    const responseData = await handleRequest(set, () =>
      api.get(`/websites/${websiteId}/experiments`)
    )
  ...
  createExperiment: async (experimentData: Partial<Experiment>) => {
    await handleRequest(
      set,
      () => api.post(`/experiments`, experimentData),
  ...
  updateExperiment: async (
    ...
    await handleRequest(
      set,
      () => api.put(`/experiments/${experimentId}`, experimentData),
      'Experiment updated successfully!'
    )
    const websiteId =
      get().selectedExperiment?.websiteId || (experimentData as any).websiteId
    if (websiteId) {
      get().fetchExperiments(websiteId)
  ...
  deleteExperiment: async (experimentId: string) => {
    const websiteId = get().experiments.find(
  ...
  updateExperimentStatus: async (experimentId: string, status: string) => {
    // Implement updateExperimentStatus
    await handleRequest(
      set,
      () => api.patch(`/experiments/${experimentId}/status`, { status }),
  ...
  declareWinner: async (experimentId: string, winningVariation: string) => {
    // Implement declareWinner
    await handleRequest(
      set,
      () =>
        api.post(`/experiments/${experimentId}/declare-winner`, {
          winningVariation,
        }),
