# SDK Implementation Guide (For Users)

To integrate the BehaveIQ SDK into your website with Zero-Flicker Personalization, you need to add a two-part script to the `<head>` section of your HTML pages. This setup ensures that personalization is applied before the page is rendered, preventing any content flicker.

## Step 1: Add the Anti-Flicker Snippet

First, add the following `<style>` tag as high up in the `<head>` of your HTML as possible. This will hide the page content until BehaveIQ has applied personalization.

```html
<head>
  <!-- Anti-Flicker Snippet (Add this first!) -->
...
## Step 2: Add the BehaveIQ SDK Scripts

Next, add the following two `<script>` tags immediately after the anti-flicker snippet.

1.  **The Initialization Script (`init.js`):** This is a dynamic script generated by our servers that contains the specific personalization rules for the current user. Replace `YOUR_API_KEY` with your actual Website API Key from the BehaveIQ dashboard.
2.  **The Library Script (`behaveiq.sdk.js`):** This is the main SDK library, which should be loaded asynchronously.
...
<head>
  ...
  <!-- BehaveIQ SDK Scripts (Add these after the snippet) -->
  <script src="http://localhost:5000/api/v1/sdk/init.js?apiKey=YOUR_API_KEY"></script>
  <script async src="http://localhost:5000/behaveiq.sdk.js"></script> <!-- Adjust this URL to where you host the SDK file -->
  
  <!-- Your other head elements -->
  <title>My Awesome Website</title>
</head>
```
**Note:** The URL for `behaveiq.sdk.js` is a placeholder. You will need to host the built SDK file (`sdk/dist/behaveiq.min.js`) and point to its location. For development, you can serve it locally.

## Complete Example

Here is a complete example of what your `<head>` section should look like:
...
<head>
    ...
    <!-- 1. BEHAVEIQ Anti-Flicker Snippet -->
    <style>body { opacity: 0 !important; }</style>

    <!-- 2. BEHAVEIQ SDK Scripts -->
    <script src="http://localhost:5000/api/v1/sdk/init.js?apiKey=YOUR_API_KEY"></script>
    <script async src="http://localhost:5000/behaveiq.sdk.js"></script>
...
```
With this setup, the `init.js` script will call `BEHAVEIQ.init()` with the correct personalization rules, which will synchronously modify the DOM and then un-hide the `<body>`, ensuring a seamless, flicker-free experience for your users.
